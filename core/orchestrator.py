...
class ZemiOrchestrator:
    ...
    async def process_command(self, user_input, user_id, channel):
        ...
        # Step 0: Deterministic router
        router_result = self._deterministic_router(user_input, user_id, channel)
...
Respond with JSON only:"""
    ...
    def _deterministic_router(self, user_input, user_id, channel):
        """
        Deterministic routing layer.
        Returns a response if handled.
        Returns None if the message should go to the LLM.
        """
        return None
    ...
    async def _execute_action(self, intent):
        ...
        if action == "memory_search":
            ...
            results = search_memory(self._last_user_input)
            if results and results[0] != "No memory found for that query":
                return "Here is what I found:\n" + "\n".join(results)
            return "I searched my memory but could not find anything about that."
        if action == "chat":
